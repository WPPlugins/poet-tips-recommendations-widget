<script> 
    ...
    function pt_get_list(result) {
        ...
        for( i=0; i < result.data.votes.length && i < 3; i++ ) {
            ...
            list.append('<li class="pt_item">' +'<a href="http://poet.tips/poet/' + pt_urlify(vote.name) + '/" target="_blank">' + vote.name + '</a>' + '</li>');
        }
    ...
    function pt_get_iframe(id, method) {
        ...
        var url = 'http://poet.tips/graph/' + encodeURIComponent(method) + '?id=' + encodeURIComponent(id);
        iframe.attr('src', url);
